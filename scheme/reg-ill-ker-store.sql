...
SELECT
    ...
    term.term_amount,
    COALESCE(pay_reg.paid_after_90d_count, pay_ill.paid_after_90d_count, 0) AS paid_after_90d_count,
    (
        cp.joined_at + INTERVAL '90 days' >= p.period_start
        AND cp.joined_at + INTERVAL '90 days' < p.period_end
        AND COALESCE(pay_reg.paid_after_90d_count, pay_ill.paid_after_90d_count, 0) >= 2
        AND cps.status = 'ACTIVE'
    )                                    AS qualifies,
    CASE cp.type WHEN 'REG' THEN er.regalia_rate_pct ELSE er.illuminati_rate_pct END AS rate_pct,
    CASE WHEN (
        cp.joined_at + INTERVAL '90 days' >= p.period_start
        AND cp.joined_at + INTERVAL '90 days' < p.period_end
        AND COALESCE(pay_reg.paid_after_90d_count, pay_ill.paid_after_90d_count, 0) >= 2
        AND cps.status = 'ACTIVE'
    ) THEN ROUND(term.term_amount * CASE cp.type WHEN 'REG' THEN er.regalia_rate_pct ELSE er.illuminati_rate_pct END, 2)
      ELSE 0
...
JOIN customer_plan_status cps
    ON cps.customer_plan_id = cp.id AND cps.end_time = '2100-01-01 00:00:00+00'
JOIN LATERAL (
    (SELECT amount FROM plan_regalia WHERE customer_plan_id = cp.id AND cp.type = 'REG' ORDER BY created_at DESC LIMIT 1)
    UNION ALL
    (SELECT amount FROM plan_illuminati WHERE customer_plan_id = cp.id AND cp.type = 'ILL' ORDER BY created_at DESC LIMIT 1)
) term(term_amount) ON TRUE
LEFT JOIN LATERAL (
    SELECT COUNT(*) FILTER (WHERE paid_at >= cp.joined_at + INTERVAL '90 days') AS paid_after_90d_count
    FROM plan_installment_regalia
    WHERE customer_plan_id = cp.id AND status = 'PAID'
