-- Employee incentive summary -- KARNATAKA & TAMIL NADU: two independent,
-- side-by-side schemes, scoped to line items billed at a Karnataka or
-- Tamil Nadu branch (both states share the same bracket thresholds and
-- rates, hence one combined file rather than two near-identical ones).
...
--   14.6% and above  ->  GOLD CAT C   Rs 30/gram (no upper bound, same
--                        open-ended top bracket as the default scheme's
--                        CAT C -- every sold item lands in exactly one of
--                        CAT 0/A/B/C).
...
-- e.g. 5.55% rounds to 5.6% (-> CAT A) and 5.54% rounds to 5.5% (-> CAT 0),
-- closing the gap between the stated ranges -- same rounding rule as the
-- default (non-region) file.
--
-- Collection is NOT part of either scheme.
-- Region here is the SALE's billing branch (invoice.location_id -> branch
-- -> address.state_code IN ('KA', 'TN')), NOT the salesperson's own branch
-- assignment -- so a salesperson who bills sales in Karnataka/Tamil Nadu
-- and another state will appear in this file for their Karnataka/Tamil
-- Nadu-billed sales only, and in the default/Kerala file for the rest. See
-- employee_mc_incentive.sql (default, everywhere except TN/KA/KL) and
-- employee_mc_incentive_kerala.sql (Kerala, default brackets, own file
-- purely for the sales-location split -- not these tighter brackets).
-- A salesperson appears only if they earned a non-zero incentive under
-- EITHER the Category or the Other scheme, from Karnataka/Tamil
-- Nadu-billed sales.
...
-- Karnataka/Tamil Nadu the same way -- by the ORIGINAL invoice's billing
-- branch.
...
sold_line_items AS (
    -- SL: original sales, sign +1 -- scoped to Karnataka/Tamil Nadu billing branch.
    SELECT
    ...
    -- to Karnataka/Tamil Nadu by the ORIGINAL invoice's billing branch.
    SELECT
        ili.id AS line_item_id,
        i.salesperson_employee_id,
        ili.product_id,
...
line_incentive AS (
    SELECT
        ...
        -- ELSE covers 14.6% and above (no upper bound) -> GOLD CAT C.
        CASE
...
employee_totals AS (
    SELECT
        ...
        -- An employee who sold from more than one KA/TN branch shows all
